...
class Tweener implements TickerProvider {
  ...
  Tweener(
    ...
    Curve tweenEase = ease ?? Ease.ease;
    CurvedAnimation curve =
        CurvedAnimation(parent: _controller!, curve: tweenEase);
    ...
    if (color != null) {
      ...
      _animationColor =
          ColorTween(begin: target.color, end: color).animate(curve);
    ...
    if (border != null) {
      shouldAnimating = true;
      _animationBorder =
          BorderTween(begin: target.border, end: border).animate(curve);
    ...
    if (borderRadius != null) {
      ...
      _animationBorderRadius =
          BorderRadiusTween(begin: target.borderRadius, end: borderRadius)
              .animate(curve);
    ...
    if (padding != null) {
      shouldAnimating = true;
      _animationPadding = EdgeInsetsTween(
              begin: target.padding as EdgeInsets, end: padding as EdgeInsets)
          .animate(curve);
    ...
    if (margin != null) {
      shouldAnimating = true;
      _animationMargin = EdgeInsetsTween(
              begin: target.margin as EdgeInsets, end: margin as EdgeInsets)
          .animate(curve);
  ...
  static Tweener to(
    ...
    Tweener tween = Tweener(target, duration,
        // data
        x: (x != null) ? x + .0 : x,
        y: (y != null) ? y + .0 : y,
  ...
  //     shouldAnimating = true;
  //     _animationPadding =
  //         EdgeInsetsTween(begin: target.padding, end: padding).animate(curve);
  //     _animationPadding.addListener(_onPaddingAnimating);
  //   }
  ...
  //     shouldAnimating = true;
  //     _animationMargin =
  //         EdgeInsetsTween(begin: target.margin, end: margin).animate(curve);
  //     _animationMargin.addListener(_onMarginAnimating);
  //   }
  ...
  void _onAnimating() {
    ...
    // if (target != null && _oldData != null) {
    if (_oldData != null) {
      if (top != null) {
        if (_oldData!.top == null) _oldData!.top = 0.0;
        target.top = _oldData!.top! + progress * (top! - _oldData!.top!);
      }
      if (left != null) {
        if (_oldData!.left == null) _oldData!.left = 0.0;
        target.left = _oldData!.left! + progress * (left! - _oldData!.left!);
      }
      if (right != null) {
        if (_oldData!.right == null) _oldData!.right = 0.0;
        target.right =
            _oldData!.right! + progress * (right! - _oldData!.right!);
      }
      if (bottom != null) {
        if (_oldData!.bottom == null) _oldData!.bottom = 0.0;
        target.bottom =
            _oldData!.bottom! + progress * (bottom! - _oldData!.bottom!);
      }

      final double oldwidth = _oldData!.width ?? 0.0;
      if (width != null) {
        target.width = oldwidth + progress * (width! - oldwidth);
      } else {
        target.width =
            oldwidth + progress * (target.state!.initSize.width - oldwidth);
      }

      final double oldheight = _oldData!.height ?? 0.0;
      if (height != null) {
        target.height = oldheight + progress * (height! - oldheight);
      } else {
        target.height =
            oldheight + progress * (target.state!.initSize.height - oldheight);
      }

      if (opacity != null)
        target.opacity =
            _oldData!.opacity + progress * (opacity! - _oldData!.opacity);
      if (rotation != null)
        target.rotation =
            _oldData!.rotation + progress * (rotation! - _oldData!.rotation);
      ...
      if (x != null && x != _oldData!.x)
        target.x = _oldData!.x + progress * (x! - _oldData!.x);
      if (y != null && y != _oldData!.y)
        target.y = _oldData!.y + progress * (y! - _oldData!.y);
    }
...
class BackInOutCurve extends Curve {
  ...
  double transform(double t) {
    ...
    final double _p1 = 1.70158;
    final double _p2 = _p1 * 1.525;
    final double result = ((t *= 2) < 1)
        ? 0.5 * t * t * ((_p2 + 1) * t - _p2)
        : 0.5 * ((t -= 2) * t * ((_p2 + 1) * t + _p2) + 2);
    return result;
  }
