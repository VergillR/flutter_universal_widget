...
class UniversalChannel extends ChangeNotifier {
  ...
  static UniversalChannel? get(String name) {
    List<UniversalChannel> results =
        _channels.where((channel) => (channel.name == name)).toList();
    if (results.length > 0) {
      return results[0];
    } else {
      return null;
    ...
    _channels.forEach((channel) {
      ...
      if (channel._widgets.length > 0) {
        channel._widgets.forEach((_widget) {
          String widgetName = _widget.name ?? _widget.hashCode.toString();
          widgetListStr += widgetName + ", ";
        });
        widgetListStr = widgetListStr.substring(0, widgetListStr.length - 2);
      }
      String attachedStr = (widgetListStr == "")
          ? " (Not attached)"
          : " (attached into UniversalWidget<$widgetListStr>)";
  ...
  UniversalChannel({String? name, UniversalWidget? widget}) {
    if (widget != null) {
      ...
      _name = "UniversalChannel<${widget.hashCode}>";
      if (widget.name != null) {
