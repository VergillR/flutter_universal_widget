...
class UniversalWidget extends StatefulWidget {
  ...
  final double x, y, rotation, opacity;
  ...
  UniversalWidget({
    ...
    this.transformOrigin = const Offset(0.5, 0.5), // transform matrix
    // display mode
  ...
  void onEnterFrame(ValueChanged<UniversalWidget> callback, {int fps = 60}) {
    ...
    _controller!._timer =
        Timer.periodic(Duration(milliseconds: miliseconds), (timer) {
      callback(this);
    });
  }
    ...
    processUpdating() {
      ...
      if (animateWhenUpdate || duration != null) {
        ...
        if (_controller!._state != null) {
          if (width != null && _controller!.width == null)
            _controller!._width = _controller!._state!._initSize.width;
          if (height != null && _controller!.height == null)
            _controller!._height = _controller!._state!._initSize.height;
        }
        ...
        Tweener.to(
          _controller!, duration ?? durationWhenUpdate,
          x: x,
          y: y,
          width: width,
          height: height,
        ...
        if (transformOrigin != null)
          _controller!.transformOrigin = transformOrigin;
  ...
  Size get size {
    Size _size = state?.context.size ?? Size(0, 0);
    double horizontal = _controller!.margin?.horizontal ?? 0;
    double vertical = _controller!.margin?.vertical ?? 0;
  ...
  Future<ByteData?> _toByte({double pixelRatio: 3.0}) async {
    RenderRepaintBoundary boundary =
        state!.context.findRenderObject() as RenderRepaintBoundary;
  ...
  void toImage(
      {double pixelRatio: 3.0, ValueChanged<Image>? onComplete}) async {
  ...
  void toBase64(
      {double pixelRatio: 3.0, ValueChanged<String>? onComplete}) async {
    if (_controller == null) return;
    // ByteData byteData = await toByte(pixelRatio: pixelRatio);
    _controller!.captureWidget(onComplete: (byteData) {
      if (byteData != null) {
  ...
  static UniversalWidget? find(String name) {
    List<UniversalWidget> results =
        _list.where((item) => (item.name == name)).toList();
    if (results.length > 0) {
      return results[0];
    } else {
  ...
  static add(UniversalWidget widget) {
    final String s = widget.name ?? '';
    if (s.isNotEmpty && exists(s)) {
      remove(s);
    }
    _list.add(widget);
  }
  ...
  static remove(String name) {
    if (exists(name)) {
      UniversalWidget widget =
          _list.where((item) => (item.name == name)).toList()[0];
      _list.remove(widget);
...
class _UniversalWidgetState extends State<UniversalWidget> {
  ...
  UniversalWidgetController? _controller = UniversalWidgetController();
  ...
  void didUpdateWidget(UniversalWidget oldWidget) {
    ...
    if (widget._controller != oldWidget._controller) {
      ...
      _controller!._mask = (widget.mask != oldWidget.mask)
          ? widget.mask
          : oldWidget._controller!.mask;
      _controller!._child = (widget.child != oldWidget.child)
          ? widget.child ?? oldWidget._controller!.child
          : oldWidget._controller!.child;

      _controller!._x =
          (widget.x != oldWidget.x) ? widget.x : oldWidget._controller!.x;
      _controller!._y =
          (widget.y != oldWidget.y) ? widget.y : oldWidget._controller!.y;
      _controller!._width = (widget.width != oldWidget.width)
          ? widget.width
          : oldWidget._controller!.width;
      _controller!._height = (widget.height != oldWidget.height)
          ? widget.height
          : oldWidget._controller!.height;
      _controller!._scale = (widget.scale != oldWidget.scale)
          ? widget.scale
          : oldWidget._controller!.scale;
      _controller!._rotation = (widget.rotation != oldWidget.rotation)
          ? widget.rotation
          : oldWidget._controller!.rotation;
      _controller!._border = (widget.border != oldWidget.border)
          ? widget.border
          : oldWidget._controller!.border;
      _controller!._borderRadius =
          (widget.borderRadius != oldWidget.borderRadius)
              ? widget.borderRadius
              : oldWidget._controller!.borderRadius;
      _controller!._transformOrigin =
          (widget.transformOrigin != oldWidget.transformOrigin)
              ? widget.transformOrigin
              : oldWidget._controller!.transformOrigin;
      _controller!._opacity = (widget.opacity != oldWidget.opacity)
          ? widget.opacity
          : oldWidget._controller!.opacity;
      _controller!._color = (widget.color != oldWidget.color)
          ? widget.color
          : oldWidget._controller!.color;
      _controller!._margin = (widget.margin != oldWidget.margin)
          ? widget.margin
          : oldWidget._controller!.margin;
      _controller!._padding = (widget.padding != oldWidget.padding)
          ? widget.padding
          : oldWidget._controller!.padding;
      _controller!._boxShadow = (widget.boxShadow != oldWidget.boxShadow)
          ? widget.boxShadow
          : oldWidget._controller!.boxShadow;

      _controller!._visible = (widget.visible != oldWidget.visible)
          ? widget.visible
          : oldWidget._controller!.visible;
      _controller!._interaction = (widget.interaction != oldWidget.interaction)
          ? widget.interaction
          : oldWidget._controller!.interaction;

      _controller!._top = (widget.top != oldWidget.top)
          ? widget.top
          : oldWidget._controller!.top;
      _controller!._bottom = (widget.bottom != oldWidget.bottom)
          ? widget.bottom
          : oldWidget._controller!.bottom;
      _controller!._left = (widget.left != oldWidget.left)
          ? widget.left
          : oldWidget._controller!.left;
      _controller!._right = (widget.right != oldWidget.right)
          ? widget.right
          : oldWidget._controller!.right;
  ...
  Widget build(BuildContext context) {
    ...
    if (_isBuiltFirstTime ||
        (_controller!.mask != _oldController!.mask) ||
        (_controller!.child != _oldController!.child) ||
        _controller!._forceRebuildWidget) {
      _isBuiltFirstTime = false;
      _controller!._forceRebuildWidget = false;
      WidgetsBinding.instance!
          .addPostFrameCallback((_) => _executeAfterFirstBuild());
    }
    ...
    if (_controller!.rotation != 0 ||
        (_controller!.rotation == 0 && _oldController!.rotation != 0)) {
      shouldApplyTransformMatrix = true;
    }

    if (_controller!.x != 0 ||
        (_controller!.x == 0 && _oldController!.x != 0)) {
      shouldApplyTransformMatrix = true;
    }

    if (_controller!.y != 0 ||
        (_controller!.y == 0 && _oldController!.y != 0)) {
      shouldApplyTransformMatrix = true;
    }

    if (_controller!.scale.dx != 1 ||
        (_controller!.scale.dx == 1 && _oldController!.scale.dx != 1)) {
      shouldApplyTransformMatrix = true;
    }

    if (_controller!.scale.dy != 1 ||
        (_controller!.scale.dy == 1 && _oldController!.scale.dy != 1)) {
      shouldApplyTransformMatrix = true;
    }
    ...
    if (_controller!.mask) {
      child = ClipRRect(
        borderRadius:
            _controller!.borderRadius ?? BorderRadius.all(Radius.circular(0)),
        child: child,
      );
    }
    ...
    if (shouldApplyTransformMatrix) {
      align = FractionalOffset(
          _controller!.transformOrigin!.dx, _controller!.transformOrigin!.dy);
      container = Transform(
        alignment: align,
    ...
    if (newOpacity != 1 || (newOpacity == 1 && _oldController!.opacity != 1)) {
      ...
      newOpacity =
          (newOpacity < 0.0) ? 0.0 : ((newOpacity > 1) ? 1 : newOpacity);

      container = Opacity(
          alwaysIncludeSemantics: true, opacity: newOpacity, child: container);
    }
    ...
    if (_parentRenderType == "RenderStack") {
      if (_controller!.top != null ||
          _controller!.left != null ||
          _controller!.right != null ||
          _controller!.bottom != null) {
...
class UniversalWidgetController extends ChangeNotifier {
  ...
  set mask(bool value) {
    _mask = value;
    // _state?._isBuiltFirstTime = true;
    if (!isDisposed) forceRebuildWidget();
    if (!isDisposed) notifyListeners();
  }
...
typedef UniversalWidgetBuilder = Widget Function(
    UniversalChannel? channel, BuildContext context);
...
class UniversalBuilder extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    _UniversalWidgetState? parentState =
        context.findAncestorStateOfType<_UniversalWidgetState>();
